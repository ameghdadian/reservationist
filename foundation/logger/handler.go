...
// Enabled reports whether the handler handles the records at the given level.
// The handler ignores records whose level is lower.
func (h *logHandler) Enabled(ctx context.Context, level slog.Level) bool {
	return h.handler.Enabled(ctx, level)
}

// WithAttrs returns a new JSONHandler whose attribute consists
...
// existing groups. The key of all subsequent attributes, whether added by With
